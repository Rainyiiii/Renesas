...
def build_target(preds, targets, cfg, device):
    ...
    #加载anchor配置
    anchors = torch.tensor(cfg["anchors"], dtype=torch.float32, device=device)
    anchors = anchors.reshape(len(preds) // 3, anchor_num, 2)
...
def compute_loss(preds, targets, cfg, device):
    ...
    lcls = torch.zeros(1, dtype=torch.float32, device=device)
    lbox = torch.zeros(1, dtype=torch.float32, device=device)
    lobj = torch.zeros(1, dtype=torch.float32, device=device)

...
void dataset_storage_progress(uint16_t stage)
{
    ...
    platform_health_t health;
    platform_services_health_get(&health);
    dashboard.touch_ready = health.touch_ready;
    dashboard.service_multicore = health.multicore_enabled;
    dashboard.service_core_online = health.m33_online;
    dashboard.service_peer_alive = health.m85_seen_by_m33;
    dashboard.service_tx_drops = health.m33_tx_drops;
...
void hal_entry(void)
{
    ...
    /* In the current project this opens local touch/weight drivers. In the
     * dual-core CPU0 project it opens IPC channel 0 and starts Cortex-M33. */
    (void) platform_services_init();
    platform_health_t platform_health;
    platform_services_health_get(&platform_health);
#if PLATFORM_SERVICES_MULTICORE
    for (uint32_t wait_ms = 0U; wait_ms < 1500U; wait_ms++)
    {
        platform_services_poll();
        platform_services_health_get(&platform_health);
        if (platform_health.m33_online)
        {
            break;
        }
    ...
    uint8_t touch_ready = platform_health.touch_ready;
    ...
    while (1) {
        ...
        platform_services_poll();
        platform_health_t latest_health;
        platform_services_health_get(&latest_health);
        if (latest_health.touch_ready != touch_ready)
        {
            touch_ready = latest_health.touch_ready;
        ...
        platform_weight_t platform_weight;
        platform_services_weight_get(&platform_weight);
        machine_control_set_weight(0U != platform_weight.valid, platform_weight.grams);
        ...
        if (touch_ready && ((frame_count & 1U) == 0U)) {
            ...
            uint8_t touch_down = platform_services_touch_read(&touch);
        ...
        if (((frame_count % DASHBOARD_REFRESH_STRIDE) == 0U) || dashboard_refresh_requested) {
            ili9488_dashboard_t dashboard = {0};
            dashboard.touch_ready = touch_ready;
            dashboard.service_multicore = latest_health.multicore_enabled;
            dashboard.service_core_online = latest_health.m33_online;
            dashboard.service_peer_alive = latest_health.m85_seen_by_m33;
            dashboard.service_tx_drops = latest_health.m33_tx_drops;
        ...
        platform_services_m85_heartbeat(frame_count, ceu_soft_fail_count);
        frame_count++;

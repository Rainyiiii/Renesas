...
#define YOLO_CROP_SIZE     (CAMERA_HEIGHT)
#define YOLO_CROP_X        ((CAMERA_WIDTH - YOLO_CROP_SIZE) / 2U)
#define YOLO_CROP_Y        (0U)
...
static void prepare_input(const uint8_t *src)
{
    ...
    {
        uint32_t sy = YOLO_CROP_Y + (y * YOLO_CROP_SIZE) / YOLO_INPUT_H;
        for (uint32_t x = 0; x < YOLO_INPUT_W; x++)
        {
            uint32_t sx = YOLO_CROP_X + (x * YOLO_CROP_SIZE) / YOLO_INPUT_W;
    ...
    for (uint32_t y = 0U; y < grid_size; y++)
    {
        for (uint32_t x = 0U; x < grid_size; x++)
        {
            ...
            for (uint32_t a = 0U; a < YOLO_ANCHOR_COUNT; a++)
            {
                ...
                const float crop_scale = (float) YOLO_CROP_SIZE / (float) YOLO_INPUT_W;
                candidate.x0 = clampf((cx - width * 0.5f) * crop_scale + (float) YOLO_CROP_X,
                                      0.0f,
                                      (float) (CAMERA_WIDTH - 1U));
                candidate.y0 = clampf((cy - height * 0.5f) * crop_scale + (float) YOLO_CROP_Y,
                                      0.0f,
                                      (float) (CAMERA_HEIGHT - 1U));
                candidate.x1 = clampf((cx + width * 0.5f) * crop_scale + (float) YOLO_CROP_X,
                                      0.0f,
                                      (float) (CAMERA_WIDTH - 1U));
                candidate.y1 = clampf((cy + height * 0.5f) * crop_scale + (float) YOLO_CROP_Y,
                                      0.0f,
                                      (float) (CAMERA_HEIGHT - 1U));

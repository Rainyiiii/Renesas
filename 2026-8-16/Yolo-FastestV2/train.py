...
if __name__ == '__main__':
    ...
    parser.add_argument('--npu-friendly', action='store_true',
                        help='Use contiguous channel splits supported by Ethos-U.')
    parser.add_argument('--workers', type=int, default=-1,
                        help='DataLoader workers; use 0 for maximum Windows/CUDA stability.')
    ...
    nw = opt.workers if opt.workers >= 0 else min([os.cpu_count(), batch_size if batch_size > 1 else 0, 8])
    # 训练集
    ...
    model = model.detector.Detector(
        cfg["classes"], cfg["anchor_num"], load_param,
        npu_friendly=opt.npu_friendly,
    ).to(device)
    ...
    for epoch in range(cfg["epochs"]):
        ...
        if epoch % 10 == 0 and epoch > 0:
            ...
            # Keep the strongest validation checkpoint across long training runs.
            if AP > best_ap + 1e-12 or (abs(AP - best_ap) <= 1e-12 and f1 > best_f1):
                best_ap = AP
                best_f1 = f1
                best_path = "weights/%s-best.pth" % cfg["model_name"]
                torch.save(model.state_dict(), best_path)
                print("Updated best model: %s (AP:%f F1:%f)" %
                      (best_path, best_ap, best_f1))

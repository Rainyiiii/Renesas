...
/* Allow extra response margin for the on-board bidirectional level translator. */
#define SDHI_PRV_RESPONSE_TIMEOUT_US                       (100000U)
...
#define SDHI_PRV_SD_INFO2_CBSY_MASK                         (0x4000U)
...
static fsp_err_t r_sdhi_wait_for_command_idle(sdhi_instance_ctrl_t * const p_ctrl);
...
volatile uint32_t g_sdhi_last_command = UINT32_MAX;
volatile uint32_t g_sdhi_last_error_info2 = 0U;
...
static void r_sdhi_access_irq_process (sdhi_instance_ctrl_t * p_ctrl, sdmmc_callback_args_t * p_args)
{
    ...
    if (0U != (info2 & (SDHI_PRV_SDHI_INFO2_CARD_CMD_ERR |
                        ...
                        SDHI_PRV_SDHI_INFO2_CARD_ILA_ERR)))
    {
        g_sdhi_last_error_info2 = info2;
    }
...
static fsp_err_t r_sdhi_command_send (sdhi_instance_ctrl_t * p_ctrl, uint32_t command, uint32_t argument)
{
    /* Keep the command number available before waiting for DAT0. */
    g_sdhi_last_command = command & 0x3FU;

    uint32_t command_index = command & 0x3FU;
    bool identification_command = (SDHI_PRV_CMD_GO_IDLE_STATE == command_index) ||
                                  (SDHI_PRV_CMD_ALL_SEND_CID == command_index) ||
                                  (SDHI_PRV_CMD_SEND_RELATIVE_ADDR == command_index) ||
                                  (SDHI_PRV_CMD_SEL_DES_CARD == command_index) ||
                                  (SDHI_PRV_CMD_IF_COND == command_index) ||
                                  (SDHI_PRV_CMD_SEND_CSD == command_index) ||
                                  (SDHI_PRV_CMD_SD_SEND_OP_COND == command_index) ||
                                  (SDHI_PRV_CMD_APP_CMD == command_index);

    /* Identification commands use only CLK/CMD. DAT0 may not yet be in the
     * transfer-state idle level, so only wait for the host command engine. */
    fsp_err_t err;
    if (identification_command)
    {
        err = r_sdhi_wait_for_command_idle(p_ctrl);
    }
    else
    {
        err = r_sdhi_wait_for_device(p_ctrl);
    }
    FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
...
{
    uint32_t timeout = SDHI_PRV_BUSY_TIMEOUT_US;
    while (0U != (p_ctrl->p_reg->SD_INFO2 & SDHI_PRV_SD_INFO2_CBSY_MASK))
    {
        FSP_ERROR_RETURN(timeout > 0U, FSP_ERR_DEVICE_BUSY);
        R_BSP_SoftwareDelay(1U, BSP_DELAY_UNITS_MICROSECONDS);
        timeout--;
    }
                                        ...
                                        sdmmc_priv_csd_reg_t * const p_csd_reg)
{
 ...
 #endif
    {
         ...
         * in the CSD is set.*/
        if ((SDHI_PRV_SD_HIGH_SPEED_CLOCK_RATE > SDHI_PRV_SD_DEFAULT_CLOCK_RATE) &&
            (SDHI_PRV_CSD_REG_CCC_CLASS_10_BIT ==
             (SDHI_PRV_CSD_REG_CCC_CLASS_10_BIT & p_csd_reg->csd_v1_b.ccc)))
        {

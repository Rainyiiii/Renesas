...
#define SERVO_PUSH_MS            (1800U) /* 推料旋转时长（原 600ms 按现场实测加大 3 倍）。 */
#define SERVO_RETURN_MS          (1800U) /* 回位反转时长。理论上与推料时长相同；
                                          * 若回位后停偏，单独微调这个值。 */
 ...
 * 送料方式：
 *   - 去前舵机的格子：识别确认后传送带继续【正转】TRAVEL_FRONT_MS
 *   - 去后舵机的格子：识别确认后传送带改为【倒转】TRAVEL_REAR_MS
 * 到位后停带，舵机朝 push_left 指定一侧旋转把物料推进左格或右格，
 * 再反向转回原位停住，然后传送带恢复正转等待下一件。
 *
 * 两个输送时间取决于摄像头到两个舵机的实际距离，装好后实测标定。 */
#define TRAVEL_FRONT_MS          (10000U) /* 判定线 → j_servo1 正转输送时间；8 秒行程仍不足，继续加长标定 */
#define TRAVEL_REAR_MS           (4000U) /* 判定线 → j_servo2 倒转输送时间，实测标定。
                                          * 计时器溢出 bug 修复后这里才是真实毫秒数——
                                          * 之前无论填多少，实际倒转都被截在约 2 秒；
                                          * 现在改这个值会真实等比例生效，放心标定。 */
...
{
    [0] = { .rear = 0U, .push_left = 1U }, /* HARM    → 前舵机 j_servo1，左格 */
    [1] = { .rear = 1U, .push_left = 1U }, /* KITCHEN → 后舵机 j_servo2，左格 */
    [2] = { .rear = 1U, .push_left = 0U }, /* OTHER   → 后舵机 j_servo2，右格 */
    [3] = { .rear = 0U, .push_left = 0U }, /* RECOVER → 前舵机 j_servo1，右格 */
};
                             ...
                             uint16_t center_x, uint16_t center_y)
{
    ...
    switch (g_status.sort_state)
    {
        ...
        case MACHINE_SORT_POSITIONING:
            ...
            if (state_elapsed(g_sort_routes[g_status.active_class & 3U].rear ?
                              TRAVEL_REAR_MS : TRAVEL_FRONT_MS))
            {
                machine_stepper_stop();
                sorting_servo_start();
            }
...
void machine_control_get_status(machine_status_t *status)
{
    if (NULL != status)
    {
        g_status.weight_valid = (uint8_t) weight_sensor_has_value();
        g_status.weight_grams = weight_sensor_get_grams();

...
/* Baud rate calculated with 0.160% error. */.baudrate_bits_b.abcse = 0,
		...
		.baudrate_bits_b.cks = 1, .baudrate_bits_b.brr = 194,
		.baudrate_bits_b.mddr = (uint8_t) 256, .baudrate_bits_b.brme = false };

/** UART extended configuration for UARTonSCI HAL driver */
const sci_b_uart_extended_cfg_t g_weight_uart_cfg_extend = { .clock =
		...
		.rx_edge_start = SCI_B_UART_START_BIT_FALLING_EDGE, .noise_cancel =
				...
				&g_weight_uart_baud_setting, .flow_control =
...
const uart_cfg_t g_weight_uart_cfg = { .channel = 4, .data_bits =
		UART_DATA_BITS_8, .parity = UART_PARITY_OFF, .stop_bits =
		UART_STOP_BITS_1, .p_callback = weight_uart_callback, .p_context = NULL,
		.p_extend = &g_weight_uart_cfg_extend,
...
const uart_instance_t g_weight_uart = { .p_ctrl = &g_weight_uart_ctrl, .p_cfg =
		&g_weight_uart_cfg, .p_api = &g_uart_on_sci_b };
...
#endif
#undef RA_NOT_DEFINED
		.p_callback = ft6336_i2c_callback, .p_context = NULL,
...
ipc_instance_ctrl_t g_ipc_m33_ctrl;

/** IPC configuration */
const ipc_cfg_t g_ipc_m33_cfg = { .channel = 0, .p_callback =
		m33_services_ipc_callback,
#if defined(NULL)
                .p_context = NULL,
#else
		.p_context = (void*) &NULL,
...
/* Instance structure to use this module. */
const ipc_instance_t g_ipc_m33 = { .p_ctrl = &g_ipc_m33_ctrl, .p_cfg =
		&g_ipc_m33_cfg, .p_api = &g_ipc_on_ipc };
